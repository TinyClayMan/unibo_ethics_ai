...
\paragraph{Training configuration.} Both CTGAN and TVAE were trained for $150$ epochs using SDV's default batch size and optimiser settings, fitted on the $50{,}000$-row reduced sample and then sampled to produce synthetic datasets of up to $300{,}000$ rows. Training cost differed sharply between the two families: on the full reduced sample, CTGAN took roughly $1$ hour $24$ minutes ($\approx 33.5$\,s/iteration) to converge, while TVAE completed the same $150$ epochs in under $5$ minutes ($\approx 2.0$\,s/iteration), consistent with the expectation that VAE training is far more stable and efficient than adversarial training. The CART synthesiser was configured with a minimum leaf size of \texttt{minibucket}~$=5$ for the baseline; fitting one tree per expanded column on the $50{,}000$-row, $312$-column matrix took about $2.5$ minutes, with sampling and post-processing adding roughly $45$ seconds.
...
\subsubsection{Utility of the Deep Generative Models}

Table~\ref{tab:deep-utility} summarises the utility metrics for CTGAN and TVAE on the census data. Both models achieve a high overall SDV quality score---$87.82\%$ for CTGAN and $88.46\%$ for TVAE---indicating that the synthetic marginals and pairwise relationships are broadly faithful to the real data. The two models reach this from different directions: TVAE has stronger per-column marginals (column-shapes score $92.45\%$ versus $88.97\%$), while CTGAN better preserves pairwise structure (column-pair-trends $86.67\%$ versus $84.47\%$). The RFF-approximated MMD is essentially identical for the two models ($\approx 0.0133$), confirming that on the numeric subspace both match the real distribution closely.

\begin{table}[ht]
\centering
\caption{Utility metrics for the SDV deep generative models on the census dataset (150 training epochs). Quality scores are from SDV's \texttt{QualityReport}; EMD is the mean per-column Wasserstein distance over the $12$ numeric columns; MMD is the RFF approximation. Higher quality scores and lower EMD/MMD are better.}
\label{tab:deep-utility}
\begin{tabular}{lcc}
\hline
...
Mean EMD (numeric cols) & $44.74$ & $47.09$ \\
RFF-MMD (approx.) & $0.0133$ & $0.0133$ \\
Training time (150 epochs) & $\sim$1h24m & $\sim$5m \\
\hline
\end{tabular}
\end{table}

The EMD results expose where utility is hardest to preserve. The mean per-column Wasserstein distance is $44.74$ for CTGAN and $47.09$ for TVAE, but this average is dominated by a few heavy-tailed financial columns. Both models reproduce bounded and low-variance columns extremely well (e.g.\ \texttt{year}, \texttt{own business or self employed}, and \texttt{veterans benefits} all below $0.2$), but struggle on sparse, outlier-heavy financial fields: \texttt{capital gains} has an EMD of $371.4$ for CTGAN and $323.3$ for TVAE, and \texttt{dividends from stocks} is harder for TVAE ($166.5$) than for CTGAN ($102.8$). This is the expected behaviour for columns that are mostly zero with rare extreme values, which generative models replicate poorly under a limited number of samples and epochs.
